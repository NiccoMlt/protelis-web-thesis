\chapter{Progettazione di sistemi web}\label{ch:web}
  ...
  Mentre nel periodo iniziale del web ciascuna pagina era inviata al client come documento statico, nel corso degli anni sono stati sviluppati linguaggi di scripting (come JavaScript e Macromedia Flash) in grado di offrire possibilità di interazione lato client.
  Questo ha permesso di ridurre la quantità di dati trasferiti tra il server web e il browser:
  ...
  Anche per quanto riguarda la componente lato server vi è stata un'evoluzione.
  ...
  \begin{figure}[htbp]
    ...
    \caption{Il controllo sulla costruzione della pagina è cambiato nel tempo}%
    \label{fig:server-evolution}
  \end{figure}

  % TODO: descrivi le architetture dei server
