...
\begin{figure}[htbp]
  ...
  \caption[
    ...
    Possibile scenario di rete in contesto urbano.\\Figura ripresa da~\cite{7274429}
  }%
  \label{fig:iot}
\end{figure}
...
Tale approccio si è ben presto rivelato inadeguato, in quanto un elevato numero di dispositivi (possibilmente eterogenei) pone diversi problemi legati all'organizzazione della rete
...
L'idea alla base è dunque di definire una modalità di deduzione del comportamento locale al singolo dispositivo a partire dal comportamento globale, di più alto livello, effettuando un \emph{mapping da globale a locale}.
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.5\textwidth]{res/fig/stack-2.png}%
  \caption[
    Principali livelli dell'\emph{Aggregate Programming}.
  ]{
    Principali livelli dell'\emph{Aggregate Programming}.\\Figura ripresa da~\cite{7274429}
  }%
  \label{fig:stack}
\end{figure}
...
% \begin{itemize}
%   \item \emph{Built-in operator} --- \texttt{b(e1, ..., en)}: % \(b(e_1, \dots, e_n)\): % ChkTeX 11, ChkTeX 26, ChkTeX 36
%     applicazione della funzione \texttt{b} agli argomenti \texttt{e1, ..., en}. % ChkTeX 11, ChkTeX 26
%     L'output è un campo ottenuto dalla valutazione dell'operatore sul campo in ingresso.
%     Possono essere funzioni \emph{stateless} matematiche, logiche o algoritmiche, ma anche sensori, attuatori o funzioni di libreria.
%   \item
%     \emph{Function definition and call} --- \texttt{def f(x1, ..., xn) \{ eb \}} % ChkTeX 11, ChkTeX 26, ChkTeX 36
%     % TODO: describe
%   \item % TODO
% \end{itemize}

% \begin{description}
%   \item[Built-in operator] \texttt{b(e1, ..., en)}\\ % ChkTeX 11, ChkTeX 26, ChkTeX 36
%     Applicazione della funzione \texttt{b} agli argomenti \texttt{e1, ..., en}. % ChkTeX 11, ChkTeX 26
%     L'output è un campo ottenuto dalla valutazione dell'operatore sul campo in ingresso.
%     Possono essere funzioni \emph{stateless} matematiche, logiche o algoritmiche, ma anche sensori, attuatori o funzioni di libreria.
%   \item[Function definition and call] \texttt{def f(x1, ..., xn) \{ eb \}}\\ % ChkTeX 11, ChkTeX 26, ChkTeX 36
%     La funzione \texttt{f} è definita con la lista di argomenti \texttt{x1, ..., xn} e il suo corpo è \texttt{eb}. % ChkTeX 11, ChkTeX 26
%     % TODO: aggiungi roba
%   \item[Time evolution] \texttt{rep(x<-v) \{ s1; ...; sn \}}\\ % ChkTeX 11, ChkTeX 26, ChkTeX 36
%     % TODO
%   \item[Neighborhood values] \texttt{nbr(s)}\\ % ChkTeX 36
%     % TODO
%   \item[Domain restriction] \texttt{if (e) \{ s1; ...; sn \} else \{ s1\textsuperscript{i}; ...; sm\textsuperscript{i} \}}\\ % ChkTeX 11, ChkTeX 26
%     % TODO
% \end{description}
...
% TODO: aggiungi:  Tali campi possono essere composti per implementare pattern di coordinazione in grado di organizzarsi in modo autonomo.
